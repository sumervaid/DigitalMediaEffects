...
library(effects)
ef9<-effect(term="socialmedia.cz*home_socialNumeric.scaled", mod=home_social_duration, xlevels=list(socialmedia.cz=c(-3, 3), home_socialNumeric.scaled=c(0, 1)))
...
ggplot(data=preddatahome_work, aes(x=socialmedia.cz, y=fit, linetype=home_socialNumeric.scaled)) + 
  ...
  ggtitle("Moderating Role of home_socialNumeric.scaled") + labs(colour="home_socialNumeric.scaled")
```
...
ranefs<-tibble(ranef(covid_wellbeing)$id)

ggplot(data=coefs_wb, aes(x=covid.scaled, fill="orange")) +
  geom_density(alpha=0.25) + 
  geom_density(data=coefs_excercise,aes(x=excercise.scaled, fill="darkblue"),alpha=0.25)+
  geom_density(data=coefs_internet,aes(x=internetUse.scaled, fill="green"), alpha=0.25)+
  xlim(-0.25, 0.25)+
