...
library(effectsize)
ef10<-effect(term="MediaUse.scaled*home_socialNumeric.scaled", mod=home_social_sm, xlevels=list(MediaUse.scaled=c(-1, 1), home_socialNumeric.scaled=c(0, 1)))
preddata_socialDuration<-as.data.frame(ef10)
preddata_socialDuration$home_socialNumeric.scaled<-as.character(preddata_socialDuration$home_socialNumeric.scaled)
ggplot(data=preddata_socialDuration, aes(x=MediaUse.scaled, y=fit, linetype=home_socialNumeric.scaled)) + 
  geom_point() +
  geom_line() +
  geom_ribbon(aes(ymin=lower, ymax=upper), alpha=.3) +
  geom_errorbar(aes(x=MediaUse.scaled, ymin=lower, ymax=upper), width=.05) +
  xlab("Social Media Use (vs. non-Use)") + scale_x_continuous(breaks=seq(-2,2,by=1)) +
...
ggplot() +
  ...
  xlim(-0.25, 0.25)+
  labs(x = "Person-Specific MLM Coefficients", y="Count") + ggtitle("Person-Specific Effects of COVID Information Seeking on Affective Wellbeing")+ 
